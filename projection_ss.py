...
for i in range(retirement_age_eric-eric_age):
    ...
    if emily_income - cc <= 180000:
        after_tax_emily = emily_income - cc - 29467 - (emily_income - 120000)*0.37
        emily_after_income_list.append(round(after_tax_emily,2))
    else:
        after_tax_emily = emily_income - cc - 51667 - (emily_income - 180000)*0.45
        emily_after_income_list.append(round(after_tax_emily,2))
    ...
    cc_index = cc*wage_inflation

    #if income > $250,000 then div 293 tax come into effect
    if emily_income < 250000:
        emily_super += (r*emily_super + cc)*0.85
        emily_super_list.append(round(emily_super,2))
    else:
        emily_super += (r*emily_super + emily_SGC)*0.85
        emily_super_list.append(round(emily_super,2))

    eric_super += (r*eric_super + eric_SGC)*0.85 + (cc-eric_SGC)*0.7
...
log_filename = "Projection_ss_calcs.csv"
